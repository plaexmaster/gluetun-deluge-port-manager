...
update_port () {
  ...
  response=$(curl -s -b "'$COOKIES'" -H "Content-Type: application/json" -d '{"method": "web.get_hosts", "params": [], "id": 1}' $DELUGE_SERVER:$DELUGE_PORT/json)
  hostid=$(echo "$response" | jq -r '.result[0][0]')
  echo "HostID set to: $hostid"
  curl -s -b "'$COOKIES'" -H "Content-Type: application/json" -d '{"method": "web.connect", "params": ["'$hostid'"], "id": 1}' $DELUGE_SERVER:$DELUGE_PORT/json
  curl -s -b "'$COOKIES'" -H "Content-Type: application/json" -d '{"method": "core.set_config", "params": [{"random_port": false}], "id": 1}' $DELUGE_SERVER:$DELUGE_PORT/json
  response1=$(curl -s -b "'$COOKIES'" -H "Content-Type: application/json" -d '{"method": "core.get_config_value", "params": ["random_port"], "id": 1}' $DELUGE_SERVER:$DELUGE_PORT/json)
  randomportstatus=$(echo "$response1" | jq -r '.result')
  echo "Random port changed to: $randomportstatus"
  curl -s -b "'$COOKIES'" -H "Content-Type: application/json" -d '{"method": "core.set_config", "params": [{"listen_ports": "'$FORMATTED_PORT'"}], "id": 1}' $DELUGE_SERVER:$DELUGE_PORT/json
