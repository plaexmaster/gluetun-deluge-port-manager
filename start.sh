...
update_port () {
  ...
  curl -s -b /gluetun/deluge_cookies.txt -H "Content-Type: application/json" -d '{"method": "core.set_config", "params": [{"listen_ports": "'[$PORT, $PORT]'"}], "id": 1}' $DELUGE_SERVER:$DELUGE_PORT/json
  rm -f "/gluetun/deluge_cookies.txt"
}
